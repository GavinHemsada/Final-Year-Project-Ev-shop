...
}> = ({ institution, onLogout, setActiveTab }) => {
    ...
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <div className="flex items-center space-x-2">
          {institution?.user_id?.profile_image ? (
            <img
              src={`${apiURL}${institution.user_id.profile_image}`}
          ...
          <div className="hidden md:block text-left">
            <p className="font-semibold text-sm truncate max-w-[150px]">
              <span className="dark:text-white">{institution?.name || "Financial Institution"}</span>
            </p>
          </div>
        </div>
        <ChevronDownIcon
